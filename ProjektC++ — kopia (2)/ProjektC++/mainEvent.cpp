...
void mainEvent::Event_GameSetup(sf::RenderWindow * app, background & TLO, controller_window & Window, game &GRA)
{
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && TLO.ksztalt[1].getGlobalBounds().contains(worldPos)) //klikniecie na pole tekstowe
	{
		a = 1;
	}

	if (Event.type == sf::Event::TextEntered && a == 1 && TLO.tekst[17].getString().getSize() < 3) //jesli kliknalem na pole i jest mniej niz 3 znaki to moge wpisywac w pole tekstowe liczby (czas tury)
	{
		unicode = Event.text.unicode;
		if (unicode >= 48 && unicode <= 57)
		{
			if (TLO.tekst[17].getString().getSize() != 0 || unicode != 48) //nie moze byc pierwsza cyfr¹ 0
			{
	...
	if (Event.type == sf::Event::TextEntered && a == 1 && TLO.tekst[17].getString().getSize() > 0) //jesli mam minimum 1 znak, to Bakcspace usuwa znak
	{
		unicode = Event.text.unicode;
		if (unicode == 8)
		{
	...
	//ustawienia wyboru GRACZ czy KOMPUTER dla 4 kolorów
	//GRACZ 1
